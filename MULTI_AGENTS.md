...
> **架构说明**：OpenClaw 运行时原生支持 SKILL.md 文件的发现和加载（Progressive Disclosure），在会话启动时扫描 skills/ 目录，命中时加载全文。longClaw 在此基础上扩展了 skill index 格式、依赖检查（requires 字段）和强制触发规则，这些扩展由 OpenClaw 的 skill 加载机制执行，不需要 CTRL 在对话中手动模拟。
...
> **架构说明**：OpenClaw 运行时（Mac mini M4）已原生内置 auto-compaction，在 session 接近上下文窗口时自动触发，保护工具调用边界。
> Layer A 是在 OpenClaw 原生 compaction **之上**的业务层偏好声明，用于在达到 OpenClaw 触发阈值之前提前做轻量摘要，减少噪声积累。
> **优先级：OpenClaw 原生 compaction 优先。若原生 compaction 已在本轮触发，Layer A 跳过，仅执行 Layer B 归档。**

### Layer A：Compression Preference（压缩偏好）

**触发信号**（满足任一，且原生 compaction 未在本轮触发）：
- 对话轮数 > 20 轮（轮数代理，不是精确 token 计数）
...
- 摘要格式：
  ...
  ```
- DEV LOG 必须显示：压缩原因 / 压缩次数累计（来自 session-state.json `compression_count`）/ 本次压缩级别
- 压缩后写入 session-state.json：`compression_count += 1`，`last_compression_at = <ISO时间戳>`
- 静默执行，不向用户额外发送提示消息
...
```
Level 1：current session / recent turns（由 Codex 上下文窗口天然覆盖）
 → 无需调用 memory_search 工具
 → Codex 在当前上下文中已有最近对话、已确认实体、当前轮决策
 → 只有 Level 1 不足时，才调用 memory_search 工具进入 Level 2
