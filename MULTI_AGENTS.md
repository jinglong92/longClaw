...
**来源标注规则（禁止混淆）**：
BRO/SIS 回复中涉及用户状态时，必须区分三种来源，不得混用：
- `[本轮]`：用户在当前会话中明确说过的信息
- `[记忆]`：memory-agent 检索到的历史记录
- `[判断]`：CTRL/BRO/SIS 基于以上两者做的推断

memory-agent 超时/失败时，只能使用 `[本轮]` 和 `[判断]`，**禁止把当前会话信息包装成记忆检索结果**。

---

## 用户偏好绑定

- 用户称呼、语气、交互偏好、安全边界以 `USER.md` 为唯一权威来源
- 路由可见性与 dev mode 展示规则以 `AGENTS.md` + `DEV_LOG.md` 为准
- 本文件只保留多代理运行偏好：默认经过 CTRL + 至少 1 个专职
